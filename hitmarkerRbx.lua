...
local Config = {
    Hitmarker = true,
    TextSize = 18,
    Duration = 0.85, -- сколько секунд висит хитмаркер
    GraySky = false
}
...
Slider(Content, "Duration", 20, 100, 85, function(v) Config.Duration = v / 100 end) -- 0.20 - 1.00 сек

Section(Content, "Visuals")
Toggle(Content, "Gray Sky", false, function(v)
    Config.GraySky = v
    if v then
        -- сохраняем ТОЛЬКО небо (освещение игры не трогаем)
        savedLighting = { skies = {} }
        for _, child in ipairs(Lighting:GetChildren()) do
            if child:IsA("Sky") or child:IsA("Atmosphere") or child:IsA("Clouds") then
                table.insert(savedLighting.skies, child:Clone())
                child:Destroy()
            end
        end

        -- только серый skybox, без fullbright
        ...
        atm.Density = 0.35
        atm.Offset = 0
        atm.Color = Color3.fromRGB(170, 170, 170)
        atm.Decay = Color3.fromRGB(150, 150, 150)
        atm.Glare = 0
        atm.Haze = 1.8
        atm.Parent = Lighting
    else
        if savedLighting then
            ...
            for _, clone in ipairs(savedLighting.skies) do
...
local ATTACK_WINDOW = 0.7 -- увеличено, чтобы меньше пропускать
...
local function markAttack()
    ...
    if mouse and mouse.Target then
        ...
        local direction = Camera.CFrame.LookVector * 800
...
local function setupCharacter(char)
    char.ChildAdded:Connect(function(child)
        if child:IsA("Tool") then
            child.Activated:Connect(markAttack)
        end
    end)
...
-- Клик / тач всегда помечаем атаку (даже без Tool — для некоторых игр)
...
local function createHitmarker(position, damage, isHeadshot)
    ...
    local duration = Config.Duration or 0.85
