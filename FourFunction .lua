...
CreateToggle(VisualsTab, "esp skeleton", function(state)
    if _G.ToggleEspSkeleton then
        _G.ToggleEspSkeleton(state)
    end
end)
...
local function removeSkeleton(player)
    if skeletonCache[player] then
        for _, line in ipairs(skeletonCache[player].Lines) do
            pcall(function() line:Remove() end)
        end
        if skeletonCache[player].Connection then
            skeletonCache[player].Connection:Disconnect()
        end
...
local function createSkeleton(player)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        if not _G.EspSkeletonEnabled then
            removeSkeleton(player)
            return
        end
...
function _G.ToggleEspSkeleton(state)
    _G.EspSkeletonEnabled = state
    if not state then
